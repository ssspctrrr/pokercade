...
public class SpawnManager : MonoBehaviour
{
    ...
    // Made public so you can see it in Inspector for debugging
    public List<card_data> currentDeck = new List<card_data>();

    // REMOVED START() so it doesn't auto-spawn
    ...
    {
        Debug.Log("SpawnManager: TriggerNextLevel called.");
        StopAllCoroutines();
    ...
    {
        Debug.Log("SpawnManager: Starting NextLevelSequence...");

        // 1. Destroy old cards
        int childCount = handSlot.childCount;
        for (int i = childCount - 1; i >= 0; i--)
        {
            if (handSlot.GetChild(i) != null)
                Destroy(handSlot.GetChild(i).gameObject);
        }

        Debug.Log("SpawnManager: Old cards destroyed. Waiting frame...");
        yield return null;

        // 2. Reset Deck
        currentDeck.Clear();
        Debug.Log($"SpawnManager: Deck Cleared. AllCardData count: {allCardData.Count}");
        
        if (allCardData.Count == 0)
        {
            Debug.LogError("ERROR: allCardData is empty! Did you assign cards in the Inspector?");
        }

        currentDeck.AddRange(allCardData);
        Debug.Log($"SpawnManager: Deck Refilled. CurrentDeck count is now: {currentDeck.Count}");

        // 3. Deal
        Debug.Log("SpawnManager: Dealing new hand...");
    ...
    IEnumerator DealHandRoutine(int amount)
    {
        for (int i = 0; i < amount; i++)
        {
            ...
            {
                Debug.LogWarning("SpawnManager: Deck ran out of cards while dealing!");
                break;
            }
    ...
    // ... (Keep your existing ArrangeHand, AnimateCardToHand, SortHandByRank, etc. below) ...
    // COPY PASTE YOUR EXISTING HELPER FUNCTIONS HERE (ArrangeHand, RefillHand, AnimateDiscard, etc)
    
    // --- IMPORTANT: Ensure RefillHand is here for PlayHandButton to use ---
    ...
    // --- Helper Functions required for the script to compile ---
    ...
    IEnumerator AnimateDiscard(List<GameObject> cardsToDiscard)
    {
        ...
        foreach (GameObject card in cardsToDiscard) Destroy(card);
    }
    ...
    IEnumerator AnimateCardToHand(GameObject card, Vector3 localTargetPos, Quaternion targetRot, CardInstance logic)
    {
        ...
        while (t < 1)
        {
            ...
            if (t >= 0.5f && logic != null) logic.SetFaceUp(true);
            yield return null; 
        }
    ...
    void SortHandByRank()
    {
        ...
        {
            CardInstance card = child.GetComponent<CardInstance>();
            if (card != null) cardsInHand.Add(card);
        }
